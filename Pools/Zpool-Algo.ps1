...
$PoolNames | ForEach-Object {
    ...
    $Payout_Currencies = @("BTC", "LTC", "DASH") | Where-Object {$Config.Pools.$PoolName.Wallets.$_}
    ...
    while (-not ($APIStatusResponse -and $APICurrenciesResponse) -and $RetryCount -gt 0) {
        try {
            ...
            if (-not $APICurrenciesResponse) {Start-Sleep $RetryDelay <#Temp fix: workaround for Zpool(Coins) anti-DDoS protect (pool allows only 6 API calls per minute)#>; $APICurrenciesResponse = Invoke-RestMethod $PoolAPICurrenciesUri -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop}
        }
        catch {
